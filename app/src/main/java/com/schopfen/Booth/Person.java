...
public class Person implements Mentionable {
    ...
    @NonNull
    @Override
    public String getTextIdsForDisplayMode(MentionIdDisplayMode mode) {
        switch (mode) {
            case FULL:
                return getLastName();
            case PARTIAL:
                String[] words = getLastName().split(" ");
                return (words.length > 1) ? words[0] : "";
            case NONE:
            default:
                return "";
    ...
    public static class PersonLoader extends MentionsLoader<Person> {
        ...
        public PersonLoader(Context res, String userId) {

            super(res);

            this.userId = userId;

            Log.e("testinguserId", userId + "   person");
        }

        @Override
        public Person[] loadData(JSONArray jsonArray) {
            Person[] data = new Person[jsonArray.length()];
            try {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    String UserID = jsonObject1.getString("UserID");
                        ...
                        data[i] = new Person("@" + FullName.replace(" ", "_"), UserID, "");
...
//                }
                ...
                Log.e("testinguserId", "Unhandled exception while parsing person JSONArray", e);
            }
...
//                        && lastName.startsWith(namePrefixes[1])
                        }
                    } else {
//                        || lastName.startsWith(namePrefixes[0])
                        if (firstName.startsWith(namePrefixes[0])) {
                            suggestions.add(suggestion);
                        }
                    }
