...
public class AdapterMainActivity extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public AdapterMainActivity(String Activity, Context mContext, ArrayList<ProductsData> productsData, CRLCallbacks callbacks) {
        ...
        switch (type) {
            case "0":
                ...
                if (!activity.equals("Explore")) {
                    ...
                    ((MyViewHolder) holder).quesUserName.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View view) {
                            Intent intent = new Intent(mContext, OthersProfileActivity.class);
                            mEditor.putString("Booth", "Other").apply();
...
//                ((MyViewHolder1) holder).viewpagerContainer.setLayoutParams(lp);
                ...
                if (products.get(i).getProductVideo().equals("")) {
                    homeHomePagerAdapter = new HomeHomePagerAdapter(activity, mContext, products.get(i).getProductImagesData());
                } else {
                    products.get(i).getProductImagesData().add(new ProductImagesData("", ""));
                    homeHomePagerAdapter = new HomeHomePagerAdapter(activity, mContext, products.get(i).getProductImagesData(), products.get(i).getProductVideo(), products.get(i).getProductVideoThumbnail());
                }
...
//                ((MyViewHolder1)holder).viewPager.setOffscreenPageLimit(2);
                ...
                span2.setSpan(new AbsoluteSizeSpan(25), 0, products.get(i).getProductPrice().length(), SPAN_INCLUSIVE_INCLUSIVE);
                ...
                if (!activity.equals("Explore")) {
                    ...
                    ((MyViewHolder1) holder).username.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            Intent intent = new Intent(mContext, BoothProfileActivity.class);
                            mEditor.putString("Booth", "Other").apply();
                    ...
                    if (sharedpreferences.getString("LastState", "").equals("user")){
                        if (sharedpreferences.getString("UserID", "").equals(products.get(i).getUserID())){
                            ...
                            ((MyViewHolder1) holder).likeIcon.setEnabled(true);
                        }
...
//                    });
            ...
            case "2":
                ...
                if (!Home_Home_Fragment.promotedProductsData.isEmpty()) {
                    HomeHomePromotedAdapter cateViewPagerAdapter = new HomeHomePromotedAdapter(activity, mContext, Home_Home_Fragment.promotedProductsData);
...
//                    cateViewPagerAdapter.notifyDataSetChanged();
    ...
    private void LoginDialog() {
        final Dialog verifyDialog = new Dialog(mContext, android.R.style.ThemeOverlay_Material_Dialog_Alert);
